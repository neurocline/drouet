...
Complete documentation is available at http://gohugo.io/.`,
    ...
    cmd.PersistentFlags().String("config", "", "config file (default is path/config.yaml|json|toml)")
    cmd.PersistentFlags().Bool("log", false, "enable Logging")
    cmd.PersistentFlags().String("logFile", "", "log File path (if set, logging enabled automatically)")
...
func initHugoBuilderFlags(cmd *cobra.Command) {
    ...
    cmd.Flags().String("cacheDir", "", "filesystem path to cache directory. Defaults: $TMPDIR/hugo_cache/")
    ...
    cmd.Flags().Bool("gc", false, "enable to run some cleanup tasks (remove unused cache files) after the build")
